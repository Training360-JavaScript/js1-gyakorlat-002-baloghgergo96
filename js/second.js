...
const checker = (paraArray, primitive) => (
    {
        exits : paraArray.some(item => item === primitive),
        index : paraArray.findIndex(item => item === primitive),
        AllElementsAreNumbers: paraArray.every(item => Number(item)===item),
        someElementsAreNumbers: paraArray.some(item => Number(item)===item)
});